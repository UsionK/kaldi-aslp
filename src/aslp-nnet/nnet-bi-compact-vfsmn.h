...
#ifndef ASLP_NNET_BI_COMPACT_VFSMN_H_
#define ASLP_NNET_BI_COMPACT_VFSMN_H_
 ...
 public:
  ...
  void GetParams(Vector<BaseFloat>* params) const {
   ...
   void GetGpuParams(std::vector<std::pair<BaseFloat *, int> > *params) {
     params->clear();
     params->push_back(std::make_pair(bfilter_.Data(), bfilter_.NumRows() * bfilter_.Stride()));
     params->push_back(std::make_pair(ffilter_.Data(), ffilter_.NumRows() * ffilter_.Stride()));
   }
  ...
  void Update(const CuMatrixBase<BaseFloat> &input,
    ...
    // KALDI_LOG << "in         " << input;
    // KALDI_LOG << "bposition_ " << bposition_;
    // KALDI_LOG << "fposition_ " << fposition_;
