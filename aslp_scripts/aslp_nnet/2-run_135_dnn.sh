...
if [ $stage -le 3 ]; then
	...
	aslp_scripts/aslp_nnet/train_scheduler.sh --train-tool "aslp-nnet-train-frame" \
        ...
        --max-iters 1 \
        --minibatch_size 1024 \
		--train-tool-opts "--gpu-id=$gpu_id --report-period=1000 --randomize=true" \
...
if [ $stage -le 4 ]; then
	...
	aslp_scripts/aslp_nnet/train_scheduler_4workers.sh --train-type "bmuf" \
        --learn-rate 0.00004 --momentum 0.9 \
        --minibatch_size 1024 \
		...
		--train-tool-opts "--report-period=1000 --randomize=true" \
